...
# CHANGE: replaced nested context/thread/model loops with an explicit list of
# (model_file, n_ctx, n_threads, model_dir) configurations. This covers:
#   (A) CTX=4096 runs that did not execute in pipeline.py and are proven safe, and
#   (B) Meta-Llama-3.1-8B Q4_K_M at all CTX/THR combos (model lives in ../extra/).
#
# Excluded from this list:
#   - All CTX=512 and CTX=2048 configurations for existing models (fully completed in pipeline.py)
#   - Phi-4-mini Q8_0, CTX=4096, THR=2 (completed in pipeline.py)
#   - Llama-3.2-1B Q4_K_M, CTX=4096, THR=2 (completed in pipeline.py)
#   - Mistral-7B Q8_0, CTX=4096 (any THR): model (7.2GB) + KV cache saturates 8GB RAM,
#     causing swap thrashing observed at 0.05 TPS. This is the edge boundary of this study.
#   - Meta-Llama-3.1-8B Q8_0: model file is 8.0GB — exceeds the 8GB ceiling before
#     any KV cache or system memory is even accounted for. Excluded entirely.
models_dir = "../models"
extra_dir  = "../extra"

configs_to_run = [
    # Llama-3.2-1B Q4_K_M: CTX=4096 THR=4 only (THR=2 already done in pipeline.py)
    ("Llama-3.2-1B-Instruct-Q4_K_M.gguf", 4096, 4, models_dir),

    # Llama-3.2-1B Q8_0: all CTX=4096 configs (neither THR was run)
    ("Llama-3.2-1B-Instruct-Q8_0.gguf",   4096, 2, models_dir),
    ("Llama-3.2-1B-Instruct-Q8_0.gguf",   4096, 4, models_dir),

    # Mistral-7B Q4_K_M: all CTX=4096 configs; Q4 model (4.1GB) + KV cache fits in 8GB
    ("Mistral-7B-Instruct-v0.3-Q4_K_M.gguf", 4096, 2, models_dir),
    ("Mistral-7B-Instruct-v0.3-Q4_K_M.gguf", 4096, 4, models_dir),

    # Phi-4-mini Q4_K_M: all CTX=4096 configs (neither THR was run)
    ("microsoft_Phi-4-mini-instruct-Q4_K_M.gguf", 4096, 2, models_dir),
    ("microsoft_Phi-4-mini-instruct-Q4_K_M.gguf", 4096, 4, models_dir),

    # Phi-4-mini Q8_0: CTX=4096 THR=4 only (THR=2 already done in pipeline.py)
    ("microsoft_Phi-4-mini-instruct-Q8_0.gguf", 4096, 4, models_dir),

    # Meta-Llama-3.1-8B Q4_K_M: full sweep across all CTX and THR combinations.
    # Model is 4.6GB — comparable to Mistral-7B Q4 (4.1GB) which ran stably at all
    # context sizes. Q8_0 (8.0GB) is excluded as it alone saturates the 8GB ceiling.
    ("Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf", 512,  2, extra_dir),
    ("Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf", 512,  4, extra_dir),
    ("Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf", 2048, 2, extra_dir),
    ("Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf", 2048, 4, extra_dir),
    ("Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf", 4096, 2, extra_dir),
    ("Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf", 4096, 4, extra_dir),
]

with open("../prompts/prompts.json", "r") as f:
    prompt_bank = json.load(f)

# master CSV for storing all results (already exists from pipeline.py — append only)
...
# CHANGE: total_runs derived from explicit config list rather than cartesian product
current_run = 0
total_runs = len(configs_to_run) * len(prompt_bank["prompts"])

# CHANGE: unpack 4-tuple to support models from different directories
for model_file, n_ctx, n_threads, base_dir in configs_to_run:
    model_path = os.path.join(base_dir, model_file)

    # record start time for model loading
    load_start = time.perf_counter()

    # initialize model (load once per config)
    ...
    for prompt_data in prompt_bank["prompts"]:
        ...
        csv_file = f"../data/individual/{name_clean}_C{n_ctx}_T{n_threads}_P{prompt_id}.csv"
