...
# gather ollama metrics (in nanoseconds)
...
with open('benchmark_metadata.txt', 'a') as f: # Use 'a' to append to what you already have
    f.writelines([
        f"\n--- Inference Metrics ---\n",
        f"Total Latency: {total_duration:.2f}s\n",
        f"Load Duration: {load_duration:.2f}s\n",
        f"Tokens (Prompt): {prompt_eval_count}\n",
        f"Tokens (Generated): {eval_count}\n",
        f"Prompt Eval Rate: {prompt_eval_rate:.2f} tokens/s\n",
        f"Token Gen Rate (TPS): {eval_rate:.2f} tokens/s\n",
        f"Time to First Token: {prompt_eval_duration:.2f}s\n"
    ])

# display results
print("\n--- Performance Summary ---")
print(f"TPS (Generation Rate): {eval_rate:.2f} tokens/s")
